...
public class EnemyViking : EnemyBase
{
    ...
    protected override void Update()
    {
        ...
        if (_isExiting)
        {
            transform.position += (Vector3)_exitDirection * _exitSpeed * Time.deltaTime;
        }
        else
        {
    ...
    private IEnumerator FireBurst()
    {
        ...
        for (int i = 0; i < _shotsPerBurst; i++)
        {
            ...
            if (PlayerHealth.Instance != null)
            {
                ...
                BulletPool.Instance.GetBullet(_bulletPrefab, transform.position, Quaternion.identity, dir * _bulletSpeed);
            }
